...
test("Fastify boundary keeps widget auth and origin protections", async () => {
  ...
  try {
    ...
    const status = await app.inject({ method: "GET", url: "/api/status", headers: { authorization: `Bearer ${config.widgetToken}` } });
    ...
    const crossSite = await app.inject({ method: "POST", url: "/api/test-write", headers: { authorization: `Bearer ${config.widgetToken}`, origin: "https://evil.example" } });
