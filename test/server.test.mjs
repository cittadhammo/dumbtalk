...
test("requested messaging and conversation features remain wired", async () => {
  ...
  assert.match(client, /function frameFocusedItem\(target, direction\)/);
  assert.match(client, /rect\.height <= available/);
  assert.match(client, /Math\.max\(120, viewport\.bottom - viewport\.top - 8\)/);
  ...
  assert.match(client, /openVideoViewer\(video\.dataset\.videoSrc, timestamp\)/);
