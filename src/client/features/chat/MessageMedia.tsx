...
function ProtectedThumbnail({ attachment, cacheKey }: { attachment: UniversalAttachment; cacheKey: string }) {
	...
	if (!source) return <span class={styles.placeholder}>{attachment.kind === 'video' ? '▶ Video' : 'Photo'}</span>;
...
export function MessageMedia({ message, onOpen }: Props) {
	const media = message.attachments.filter((attachment) => attachment.kind === 'image' || attachment.kind === 'video');
	if (!media.length) return null;

	return (
		<span class={styles.mediaGrid}>
...
export function MediaViewer({
	...
	return (
		<main class={styles.viewer}>
			...
			<section class={`${styles.viewerBody} ${zoomed ? styles.zoomed : ''}`}>
			...
			<div class={styles.viewerControls}>
				{media.length > 1 && <FocusButton id="viewer-previous" type="button" onClick={() => onChange(-1)}>‹</FocusButton>}
				{attachment.kind === 'image' && <FocusButton id="viewer-zoom" type="button" onClick={() => setZoomed((value) => !value)}>{zoomed ? 'Fit' : 'Zoom'}</FocusButton>}
				{media.length > 1 && <FocusButton id="viewer-next" type="button" onClick={() => onChange(1)}>›</FocusButton>}
			</div>
			<FocusButton id="viewer-back" type="button" class={styles.back} onClick={onBack}>Back</FocusButton>
		</main>
	);
}
