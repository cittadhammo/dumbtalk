...
const Usage = Type.Object({
	...
	launches: Type.Array(Type.Number()),
	nudges: Type.Record(Type.String(), Type.Union([Type.Boolean(), Type.Number()])),
	lastLaunch: Type.Number(),
});
const MindfulInput = Type.Object({
			...
			async (request) => {
				...
				const saved = await state.update((current) => {
					current.mindfulUsage[day] = {
						...
						launches: usage.launches.filter(Number.isFinite).slice(-24),
						nudges: usage.nudges,
						lastLaunch: Math.max(0, Math.floor(usage.lastLaunch)),
					};
					const oldest = new Date(Date.now() - 7 * 86_400_000).toLocaleDateString('en-CA', {
						timeZone: 'Europe/London',
					});
					for (const key of Object.keys(current.mindfulUsage))
						if (key < oldest) delete current.mindfulUsage[key];
				});
