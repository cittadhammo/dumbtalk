...
async function loadMindfulUsage() {
  const day = usageDay();
  try {
    const remote = await request(`/api/mindful?day=${encodeURIComponent(day)}`);
    const saved = remote.usage || JSON.parse(localStorage.getItem(MINDFUL_KEY) || "{}");
    if (saved.day === day || remote.usage) return { day, checks: Number(saved.checks) || 0, activeMs: Number(saved.activeMs) || 0, launches: Array.isArray(saved.launches) ? saved.launches : [], nudges: saved.nudges || {}, lastLaunch: Number(saved.lastLaunch) || 0 };
  } catch {}
  return { day, checks: 0, activeMs: 0, launches: [], nudges: {}, lastLaunch: 0 };
}
function saveMindfulUsage() {
  if (!state.mindful) return;
  localStorage.setItem(MINDFUL_KEY, JSON.stringify(state.mindful));
  const usage = { ...state.mindful }; delete usage.day; delete usage.lastTick; delete usage.openedNow;
  request("/api/mindful", { method: "POST", body: JSON.stringify({ day: state.mindful.day, usage }) }).catch(() => {});
}
...
function screen(title, content, className = "") { clearProtectedMedia(); const heading = title === "SigDumb" ? `<span class="brand-title"><img src="/sigdumb.png" alt="">SigDumb</span>` : escapeHtml(title); app.innerHTML = `<section class="screen ${className}"><header>${heading}${usageBadge()}</header>${content}</section>`; app.scrollTop = 0; }
function actionError(error) { const target = document.querySelector("#action-error"); if (target) target.textContent = error.message; }

async function boot() {
  state.view = "loading"; app.innerHTML = `<section class="center"><img class="brand-logo" src="/sigdumb.png" alt=""><p>Starting SigDumb…</p></section>`; setSoftkeys();
  try {
    if (!WIDGET_TOKEN) throw new Error("This widget is not configured.");
    ...
    const message = error.status === 404 ? "This widget is not configured." : error.message;
...
function linkScreen() {
  state.view = "link"; screen("Link Signal", `<div class="center"><p>Scan a link QR from Signal → Settings → Linked devices.</p><button id="link" class="action focusable">Generate QR</button><p id="link-error" class="error"></p></div>`); setSoftkeys("", "Select", "Exit");
  document.querySelector("#link").addEventListener("click", beginLink); focusFirst();
}
async function beginLink() {
  try {
    const link = await request("/api/link/start", { method: "POST", body: "{}" });
    state.view = "linking";
    app.innerHTML = `<section class="linking"><h2>Scan on your phone</h2><img class="qr" src="${link.qr}" alt="Signal device linking QR code"><p id="link-status" role="status">Waiting for Signal…</p></section>`;
    setSoftkeys("", "", "Cancel");
    await request("/api/link/finish", { method: "POST", body: JSON.stringify({ uri: link.uri }) }); await boot();
  } catch (error) { const target = document.querySelector("#link-status") || document.querySelector("#link-error"); if (target) target.textContent = error.message; }
}
...
function avatarMarkup(item) { return item.noteToSelf ? `<span class="avatar note-avatar" aria-label="Note to Self">🔖</span>` : `<span class="avatar"><span>${escapeHtml(initials(item.name))}</span><img data-protected-src="${escapeHtml(item.avatar)}" alt=""></span>`; }
...
async function conversations(restoreId, showArchived = state.showingArchived || false) {
  ...
  screen(payload.showingArchived ? "Archived" : "SigDumb", rows || `<p class="empty">${payload.showingArchived ? "No archived conversations" : "No conversations yet"}</p>`);
...
function openVideoViewer(src, timestamp = null) {
  ...
  clearProtectedMedia(); app.innerHTML = `<section class="video-viewer"><video ${src.startsWith("/api/") ? `data-protected-src="${src}"` : `src="${escapeHtml(src)}"`} controls autoplay playsinline></video></section>`; hydrateProtectedMedia(app);
  setSoftkeys("", "Play/Pause", "Back");
  const video = document.querySelector(".video-viewer video"); if (video.src) video.play().catch(() => {});
}
...
function reactionsHtml(reactions) {
  ...
  const grouped = new Map(); for (const reaction of reactions) grouped.set(reaction.emoji, (grouped.get(reaction.emoji) || 0) + 1);
  return `<span class="reactions">${[...grouped].map(([emoji, count]) => `${escapeHtml(emoji)}${count > 1 ? count : ""}`).join(" ")}</span>`;
}
...
function messageHtml(message) {
  ...
  const sender = state.selected.kind === "group" && message.direction === "in" ? `<b class="sender">${escapeHtml(message.sender)}</b>` : "";
...
function reactionPicker(message) {
  ...
  const groups = Object.entries(EMOJI_GROUPS).map(([name, emojis]) => `<h3>${name}</h3><div class="emoji-grid">${emojis.split(" ").map(emoji => `<button class="emoji focusable" data-emoji="${emoji}">${emoji}</button>`).join("")}</div>`).join("");
...
function settingsScreen() {
  ...
  screen("Settings", `<form id="settings-form" class="menu-list"><p class="menu-heading">Privacy & appearance</p><button type="button" id="read-toggle" class="action menu-action focusable"><span class="menu-icon">✓</span><span>Read receipts</span><span class="menu-value">${state.settings.sendReadReceipts ? "On" : "Off"}</span></button><button type="button" id="typing-toggle" class="action menu-action focusable"><span class="menu-icon">•••</span><span>Typing indicators</span><span class="menu-value">${state.settings.sendTypingIndicators ? "On" : "Off"}</span></button><button type="button" id="preview-toggle" class="action menu-action focusable"><span class="menu-icon">↗</span><span>Link previews</span><span class="menu-value">${state.settings.linkPreviews ? "On" : "Off"}</span></button><label class="setting-label setting-card"><span>Default disappearing timer</span><select id="default-expiration" class="field focusable"><option value="0">Off</option><option value="300">5 minutes</option><option value="3600">1 hour</option><option value="86400">1 day</option><option value="604800">1 week</option></select></label><button class="action primary focusable">Save settings</button><p class="menu-heading">Signal service</p><div class="setting-card status-card"><strong>signal-cli ${escapeHtml(state.signalCli.version || "bundled")}</strong><span>Update status: ${escapeHtml(state.signalCli.update || "unknown")}</span>${state.signalCli.error ? `<small>${escapeHtml(state.signalCli.error)}</small>` : ""}</div><p id="action-error" class="error"></p></form>`); setSoftkeys("", "Select", "Back");
  document.querySelector("#default-expiration").value = String(state.settings.defaultExpiration || 0);
  ...
  document.querySelector("#settings-form").addEventListener("submit", async event => { event.preventDefault(); try { const payload = await request("/api/settings", { method: "POST", body: JSON.stringify({ sendReadReceipts: state.settings.sendReadReceipts, sendTypingIndicators: state.settings.sendTypingIndicators, linkPreviews: state.settings.linkPreviews, defaultExpiration: Number(document.querySelector("#default-expiration").value) }) }); state.settings = payload.settings; await conversations(); } catch (error) { actionError(error); } }); focusFirst();
}
function chatOptions() {
  stopRefresh(); state.roomScroll = app.scrollTop; state.view = "chat-options"; const archived = Boolean(state.selected.archived); const current = state.selected.expiration || 0;
  screen("Chat options", `<div class="menu-list"><p class="menu-heading">Message</p>${state.capabilities.voiceNotes ? `<button id="record-voice" class="action menu-action focusable"><span class="menu-icon">●</span><span>Record voice note</span><span class="chevron">›</span></button>` : ""}${state.capabilities.polls ? `<button id="create-poll" class="action menu-action focusable"><span class="menu-icon">▥</span><span>Create poll</span><span class="chevron">›</span></button>` : ""}${state.capabilities.pins ? `<button id="view-pins" class="action menu-action focusable"><span class="menu-icon">⌖</span><span>View pinned messages</span><span class="chevron">›</span></button>` : ""}<p class="menu-heading">Conversation</p>${state.selected.invited ? `<button id="accept-invite" class="action menu-action focusable"><span class="menu-icon">✓</span><span>Accept group invitation</span></button><button id="decline-invite" class="action menu-action danger focusable"><span class="menu-icon">×</span><span>Decline invitation</span></button>` : ""}${state.selected.kind === "group" ? `<button id="group-settings" class="action menu-action focusable"><span class="menu-icon">♟</span><span>Group settings</span><span class="chevron">›</span></button>` : !state.selected.noteToSelf ? `<button id="safety-number" class="action menu-action focusable"><span class="menu-icon">◇</span><span>Safety number</span><span class="chevron">›</span></button>` : ""}${state.selected.messageRequest ? `<button id="accept-request" class="action menu-action focusable"><span class="menu-icon">✓</span><span>Accept message request</span></button><button id="delete-request" class="action menu-action danger focusable"><span class="menu-icon">×</span><span>Delete message request</span></button>` : ""}<button id="archive-chat" class="action menu-action focusable"><span class="menu-icon">${archived ? "↥" : "↧"}</span><span>${archived ? "Unarchive" : "Archive"} chat</span></button>${!state.selected.noteToSelf ? `<button id="block-chat" class="action menu-action danger focusable"><span class="menu-icon">⊘</span><span>${state.selected.blocked ? "Unblock" : "Block"} ${state.selected.kind === "group" ? "group" : "contact"}</span></button>` : ""}<label class="setting-label setting-card"><span>◷ Disappearing messages</span><select id="chat-expiration" class="field focusable"><option value="0">Off</option><option value="300">5 minutes</option><option value="3600">1 hour</option><option value="86400">1 day</option><option value="604800">1 week</option></select></label><button id="save-expiration" class="action primary focusable">Save timer</button><p id="action-error" class="error"></p></div>`); setSoftkeys("", "Select", "Back"); document.querySelector("#chat-expiration").value = String(current);
  document.querySelector("#record-voice")?.addEventListener("click", voiceRecorderScreen);
  if (document.querySelector("#record-voice") && navigator.hasFeature) navigator.hasFeature("AudioCapture").then(supported => { if (!supported) document.querySelector("#record-voice")?.remove(); }).catch(() => {});
  document.querySelector("#create-poll")?.addEventListener("click", pollComposer);
  document.querySelector("#view-pins")?.addEventListener("click", pinnedMessages);
  document.querySelector("#safety-number")?.addEventListener("click", safetyNumberScreen);
  document.querySelector("#group-settings")?.addEventListener("click", groupSettingsScreen);
  document.querySelector("#accept-request")?.addEventListener("click", async () => { try { await request("/api/message-request", { method: "POST", body: JSON.stringify({ kind: state.selected.kind, target: state.selected.target, type: "accept" }) }); state.selected.messageRequest = false; chatOptions(); } catch (error) { actionError(error); } });
  document.querySelector("#delete-request")?.addEventListener("click", async () => { try { await request("/api/message-request", { method: "POST", body: JSON.stringify({ kind: state.selected.kind, target: state.selected.target, type: "delete" }) }); await conversations(); } catch (error) { actionError(error); } });
  document.querySelector("#accept-invite")?.addEventListener("click", () => updateGroup({}));
  document.querySelector("#decline-invite")?.addEventListener("click", async () => { try { await request("/api/group/leave", { method: "POST", body: JSON.stringify({ groupId: state.selected.target }) }); await conversations(); } catch (error) { actionError(error); } });
  document.querySelector("#block-chat")?.addEventListener("click", async () => { try { await request("/api/conversation/block", { method: "POST", body: JSON.stringify({ kind: state.selected.kind, target: state.selected.target, blocked: !state.selected.blocked }) }); state.selected.blocked = !state.selected.blocked; chatOptions(); } catch (error) { actionError(error); } });
  document.querySelector("#archive-chat").addEventListener("click", async () => { try { await request("/api/conversation/archive", { method: "POST", body: JSON.stringify({ conversationId: state.selected.id, archived: !archived }) }); state.selected.archived = !archived; await conversations(null, state.showingArchived); } catch (error) { actionError(error); } });
  document.querySelector("#save-expiration").addEventListener("click", async () => { try { const payload = await request("/api/conversation/expiration", { method: "POST", body: JSON.stringify({ kind: state.selected.kind, target: state.selected.target, expiration: Number(document.querySelector("#chat-expiration").value) }) }); state.selected.expiration = payload.expiration; await openConversation(state.selected.id); } catch (error) { actionError(error); } }); focusFirst();
}
