...
function login(message = "") {
  ...
  app.innerHTML = `<section class="center login-screen"><img class="brand-logo" src="/sigdumb.png" alt=""><h1>SigDumb</h1><p class="hint">Signal for CloudPhone</p><form id="login-form"><input class="field focusable" name="password" type="password" autocomplete="current-password" placeholder="Password" required><p class="error">${escapeHtml(message)}</p></form></section>`;
...
function avatarMarkup(item) { return item.noteToSelf ? `<span class="avatar note-avatar" aria-label="Note to Self">🔖</span>` : `<span class="avatar"><span>${escapeHtml(initials(item.name))}</span><img src="${escapeHtml(item.avatar)}" alt=""></span>`; }
